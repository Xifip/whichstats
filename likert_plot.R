...
likert_plot_by_factor <- function(data, factor, factor_labels){
  ...
  for (i in 1:27){    
   ...
   l3 <- likert(likert.df.resp[,11:15], grouping = data_factor)
   l4 <- likert(likert.df.resp[,16:20], grouping = data_factor)
   l5 <- likert(likert.df.resp[,21:25], grouping = data_factor)
   l6 <- likert(likert.df.resp[,26:27], grouping = data_factor)
   l_all <- likert(likert.df.resp, grouping = data_factor)
   lg <- list(l1, l2, l3, l4, l5, l6, l_all)
